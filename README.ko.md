...
**컨텐츠 언어 정책 선택** — 설치 시점에 커밋 메시지·PR 본문·문서의 허용 언어를 English / English + Korean(한글) / 제한 없음 중에서 선택합니다 (`CLAUDE_CONTENT_LANGUAGE=english|korean_plus_english|any`). 기본값 english는 기존 동작을 그대로 유지합니다.
...
두 installer (`install.sh`, `install.ps1`)는 설치 타입 선택 후에 컨텐츠 언어 정책을 묻습니다. 선택된 값은 `~/.claude/settings.json`의 `env.CLAUDE_CONTENT_LANGUAGE`에 기록되어 런타임 검증자가 무엇을 수용할지 결정합니다.

| 값 | 검증자가 수용하는 범위 | 규칙 문서 phrase |
|----|----------------------|-----------------|
| `english` (기본) | ASCII printable + whitespace | `English` |
| `korean_plus_english` | ASCII + 한글 음절 / 자모 / 호환 자모 | `English or Korean` |
| `any` | 언어 검증 건너뜀 | `any language` |
