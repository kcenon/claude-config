...
Build-system, package-manager, container and CI configuration samples. Companion to `.claude/rules/project-management/build.md`, which keeps the principles.

> **Loading**: Lives in `.claude/reference/`, outside the auto-loaded `.claude/rules/` tree, so it is never injected automatically. Load with `@load: reference/build-examples`.
