...
Worked SOLID, layered-architecture, design-pattern and microservice samples. Companion to `.claude/rules/api/architecture.md`, which keeps the principles.

> **Loading**: Lives in `.claude/reference/`, outside the auto-loaded `.claude/rules/` tree, so it is never injected automatically. Load with `@load: reference/architecture-examples`.
