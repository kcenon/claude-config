...
# Get Claude Code version (best-effort; pipefail-safe).
CC_VERSION=""
...
for v in $KNOWN_CACHE_BUG_VERSIONS; do
    if [ "$CC_VERSION" = "$v" ]; then
        ...
        echo "[VersionCheck] WARNING: Claude Code v${CC_VERSION} has known cache bugs (resume cache regression, sentinel replacement). See: https://github.com/anthropics/claude-code/issues/34629 — $TIMESTAMP" >> "$LOG_FILE" 2>/dev/null || true
        break
    fi
done
