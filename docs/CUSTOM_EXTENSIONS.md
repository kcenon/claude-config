...
### Doc-Index Manifest Drift Guard

**Type**: Repository-internal convention

`docs/.index/manifest.yaml` is generated by `/doc-index`, but stale path and byte-size entries can otherwise ship unnoticed after documentation moves. `scripts/validate-doc-index.sh` validates the generated manifest against the working tree:
...
The guard intentionally stays narrow. It does not replace `/doc-index`; it only fails when the checked-in manifest is no longer a faithful index of the files it claims to cover.

**CI enforcement**: `.github/workflows/validate-skills.yml` runs the validator tests and then `scripts/validate-doc-index.sh` for every relevant PR. When documentation changes legitimately alter sizes or covered paths, regenerate `docs/.index/manifest.yaml` with `/doc-index` before opening the PR.
