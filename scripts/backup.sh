...
if [ "$REPLACE" = "y" ]; then
    # enterprise 디렉토리 업데이트 (copy-then-swap: 복사 실패 시 기존 백업 보존)
    if [ -d "$TEMP_BACKUP/enterprise" ] && [ -n "$(ls -A "$TEMP_BACKUP/enterprise" 2>/dev/null)" ]; then
        ENT_STAGING="$BACKUP_DIR/enterprise.new.$$"
    ...
    # global 디렉토리 업데이트 (copy-then-swap: 복사 실패 시 기존 백업 보존)
    if [ -d "$TEMP_BACKUP/global" ] && [ -n "$(ls -A "$TEMP_BACKUP/global" 2>/dev/null)" ]; then
        GLOBAL_STAGING="$BACKUP_DIR/global.new.$$"
    ...
    # project 디렉토리 업데이트 (copy-then-swap: 복사 실패 시 기존 백업 보존)
    if [ -d "$TEMP_BACKUP/project" ] && [ -n "$(ls -A "$TEMP_BACKUP/project" 2>/dev/null)" ]; then
        PROJ_STAGING="$BACKUP_DIR/project.new.$$"
