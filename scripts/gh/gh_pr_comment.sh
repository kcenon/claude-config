...
#
# Usage:
#   ./gh_pr_comment.sh -n 42 -b "Comment text"          # Auto-detect repo
#   ./gh_pr_comment.sh -r owner/repo -n 42 -b "LGTM"
#   ./gh_pr_comment.sh -n 42 -e                          # Open editor
#   ./gh_pr_comment.sh -n 42 -b "LGTM" --json           # JSON output
#   ./gh_pr_comment.sh -n 42 -b "LGTM" --quiet          # Minimal output
#   ./gh_pr_comment.sh -h                                 # Show help
#
...
show_help() {
    ...
    echo "  $0 -n 42 -b \"LGTM, merging now\""
    echo "  $0 -r kcenon/thread_system -n 10 -b \"Please fix the typo\""
...
main() {
    ...
    print_info "PR:         #$number"

    # Build gh command
    local -a cmd=(gh pr comment "$number" --repo "$repo")
